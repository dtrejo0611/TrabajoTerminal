...
class CameraStream:
    def __init__(self, port):
        ...
        # Iniciamos el hilo inmediatamente sin llamar a self.cap.read() aquí.
        # Esto evita que todo el programa se congele si la cámara aún no transmite.
    ...
    def update(self):
        ...
        while not self.stopped:
            if self.cap is None or not self.cap.isOpened():
                time.sleep(1)  # Dar un respiro antes de intentar reconectar
                self._conectar_camara()
                continue
            ...
            with self.lock:
                if grabbed and frame is not None:
                    ...
                    # Si la transmisión se corta, forzamos el frame a None.
                    # Esto evita procesar una imagen congelada y mandar comandos 'TARGET' falsos.
                    self.grabbed = False
                    self.frame = None
                    # Forzar la reconstrucción del pipeline para recuperar la señal.
                    self._conectar_camara()

    def read(self):
        with self.lock:
            if self.frame is None:
                # Usar .copy() evita colisiones de memoria (race conditions) con YOLO
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def actualizar_displays(self, frames_anotados):
        ...
        for i, display in enumerate(displays):
            ...
            # Validación segura de la longitud de los resultados
