...
STREAM_COMMAND = (
    ...
    f"--bitrate 1500000 --intra 10 --inline --codec h264 -o - | "
    f"gst-launch-1.0 -v fdsrc ! h264parse ! rtph264pay config-interval=1 pt=96 ! "
    f"udpsink host={CLIENT_IP} port=5002 sync=false async=false"
)
...
class ServoController:
    ...
    def __init__(self, channels=16):
        ...
        try:
            ...
            # --- NUEVO: CONFIGURACIÓN ESPECÍFICA POR CANAL ---
            # Diccionario con el formato -> canal: (pulso_minimo, pulso_maximo)
            # Solo agregamos los canales que necesitan corrección.
            calibraciones = {
                0: (500, 2400),  # El servo que empezaba en 50° (ajusta el 500 según tus pruebas)
                1: (750, 2500), # El canal 1 no lo ponemos. La librería usará sus valores por defecto
                # y seguirá funcionando perfecto como hasta ahora.
            }
            
            # Aplicamos la calibración solo a los servos especificados
